export const useUser = () => {
    const updateUser = async (user_id: number, { email, password }: { email: string; password: string }) => {
        try {
            const emailToUpdate = email.trim().length > 0 ? email.trim() : undefined;
            const passwordToUpdate = password.trim().length > 0 ? password.trim() : undefined;
