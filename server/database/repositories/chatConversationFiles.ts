...
export async function readAllChatConversationFilesOfChatConversationMessage(
  chat_conversation_id: ReadChatConversationFile['chat_conversation_id'],
  chat_conversation_message_id: ReadChatConversationFile['chat_conversation_message_id']
) {
  const chatConversationFiles = await db
    .select()
    .from(chat_conversation_file)
    .where(
      and(
        eq(chat_conversation_file.chat_conversation_id, chat_conversation_id),
        eq(
          chat_conversation_file.chat_conversation_message_id,
...
export async function deleteChatConversationFilesByMessageId(
  chat_conversation_message_id: ReadChatConversationFile['chat_conversation_message_id']
) {
