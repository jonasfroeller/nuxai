...
export const IS_SERVERLESS =
    JSON.parse(process.env.IS_SERVERLESS ?? 'false') ??
...
export const LOG_SQL_QUERIES = JSON.parse(process.env.LOG_SQL_QUERIES ?? 'false') ?? false;
export const LOG_BACKEND = JSON.parse(process.env.LOG_BACKEND ?? 'false') ?? false;
