import { z } from "zod";
import { passwordSchema, confirmPasswordSchema, addPasswordFieldIssue } from "./password";
...
function validateInput(input: string, schema: z.ZodString | z.ZodEffects<any>) {
    ...
    const errorMessages = validationResult.error.issues.map((issue: any) => issue.message);
